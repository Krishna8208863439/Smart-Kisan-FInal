...
const KisanChat = () => {
  ...
  // Main sending handler using OpenAI ChatGPT API via backend /chat endpoint
  const handleSendMessage = async (textToSend) => {
    if (loading) return;

    const inputContent = textToSend || inputText;
    if (!inputContent || !inputContent.trim() && !base64Image) return;
    ...
    try {
      // Build capped history (last 10 turns) formatted as role: 'user'|'assistant'
      ...
      const weatherSummary = weather 
        ? `${weather.temp}°C · ${weather.forecast || 'Weather Synced'}` 
        : undefined;
      ...
      if (!replyText) {
        throw new Error(data?.error || "AgriExpert couldn't respond — empty reply received.");
      }
      ...
      addHistoryEntry({
        ...
        icon: "🤖",
        summary: replyText.slice(0, 100).replace(/\*\*/g, "").replace(/\n/g, " "),
        data: {
          mode: activeMode,
          language,
          source: data?.source || "chatgpt",
      ...
      console.error("AgriExpert UI error:", error);
      const parsedErr = extractErrorMessage(error, "Failed to connect to AgriExpert service. Please check your connection.");
      const errMessage = parsedErr.includes("AgriExpert couldn't respond")
        ? parsedErr
        : `AgriExpert couldn't respond — ${parsedErr}`;
        ...
        <main className="chat-main">
          ...
          <div className="chat-history">
            {messages.map((m, idx) => (
              <div 
                key={idx} 
                className={`chat-bubble ${m.sender === "user" ? "chat-bubble-user" : "chat-bubble-ai"} ${m.isError ? "chat-bubble-error" : ""}`}
              >
                ...
                {m.source && (
                  <div style={{ fontSize: 9, opacity: 0.65, textAlign: "right", marginTop: 6 }}>
                    {language === 'mr' ? 'द्वारे' : 'via'} {m.source === "chatgpt" ? "OpenAI ChatGPT API" : m.source === "claude-haiku" || m.source === "claude" ? "Anthropic Claude API" : m.source === "gemini" ? "Google Gemini" : "AgriExpert AI"}
                  </div>
                )}
              </div>
            ))}
